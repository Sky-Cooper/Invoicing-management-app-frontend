        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto no-scrollbar p-8 lg:p-14 bg-slate-50/30">
            ...
            {typeof rawError === 'string' && (
                <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-600 font-bold text-sm text-center">
                    {rawError}
                </div>
            )}
              ...
              <div className="lg:col-span-9 space-y-12">
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <InputField label="Poste Occupé" icon={<Briefcase size={20}/>} value={formData.job_title} 
                      onChange={(v: any) => setFormData({...formData, job_title: v})} error={getFieldError('job_title')} required />
                    
                    <InputField label="Numéro CIN" icon={<ShieldCheck size={20}/>} value={formData.cin} 
                    ...
                    <div className="space-y-2 flex-1 group relative">
                        ...
                        <div className="relative">
                            ...
                            <select 
                                required={isCompanyAdmin} // Only required if admin
                            ...
                            {isCompanyAdmin && <ChevronDown size={18} className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" />}
                        </div>
                        {!isCompanyAdmin && (
                            <p className="ml-6 text-[9px] font-bold text-slate-400 uppercase tracking-wide">
                                * Verrouillé: {myDeptId ? "Assigné auto." : "Aucun dept. trouvé"}
                            </p>
                        )}
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <InputField label="Date d'embauche" type="date" icon={<Calendar size={20}/>} value={formData.hire_date} 
                      onChange={(v: any) => setFormData({...formData, hire_date: v})} error={getFieldError('hire_date')} />

                    {!initialData && (
                        <InputField label="Mot de passe" type="password" icon={<Key size={20}/>} value={formData.user.password} 
          ...
          <div className="bg-white px-12 py-10 border-t border-slate-100 flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-6 w-full md:w-auto">
              ...
              >
                ...
                {success ? 'Employé Enregistré' : initialData ? 'Valider les modifications' : 'Confirmer Recrutement'}
              </button>
            </div>
          </div>
        </motion.div>
