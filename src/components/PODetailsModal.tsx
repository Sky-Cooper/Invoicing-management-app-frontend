...
export const PODetailsModal = ({ isOpen, onClose, po }: Props) => {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case 'COMPLETED': return 'bg-slate-900 text-white border-slate-700'; // Special style for Completed
      case 'CANCELLED': return 'bg-rose-50 text-rose-700 border-rose-200';
      default: return 'bg-slate-50 text-slate-600 border-slate-200'; // Draft
    }
  };
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase border ${getStatusStyle(po.status)}`}>
                      {getStatusIcon(po.status)} {po.status}
                    </span>
                  </div>
                  <p className="text-slate-500 text-sm font-medium flex items-center gap-2">
                    <User size={14} className="text-slate-400" /> 
                    <span className="font-bold text-slate-700">{po.client_name || "Fournisseur Inconnu"}</span>
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 lg:p-10 space-y-10 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-6 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">N° BC</label>
                  <p className="text-slate-900 font-bold flex items-center gap-2 text-sm">
                    <Hash size={14} className="text-slate-900" /> {po.po_number}
                  </p>
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Date Commande</label>
                  <p className="text-slate-900 font-bold flex items-center gap-2 text-sm">
                    <Calendar size={14} className="text-slate-900" /> {po.issued_date}
                  </p>
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Livraison Prévue</label>
                  <p className="text-slate-900 font-bold flex items-center gap-2 text-sm">
                    <Truck size={14} className="text-slate-900" /> {po.expected_delivery_date}
                  </p>
                </div>
              </div>
              ...
              <div>
                 ...
                 <div className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm">
                   <p className="text-slate-600 text-sm leading-relaxed">
                     {po.project_description || "Aucune description fournie."}
                   </p>
                 </div>
              </div>
              ...
              {/* ITEMS TABLE */}
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Articles commandés</label>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-12 pt-4">
                ...
                <div className="w-full md:w-80 space-y-4">
                  <div className="flex justify-between text-xs text-slate-500 font-medium">
                    ...
                    <span className="font-bold text-slate-900">{po.subtotal} DH</span>
                  </div>
                  ...
                  <div className="p-6 bg-slate-900 rounded-2xl text-white mt-4 shadow-xl shadow-slate-200">
                    <div className="flex justify-between items-end">
                        ...
                        <span className="text-2xl font-black tracking-tight">{po.total_ttc} <span className="text-sm font-medium text-slate-400">DH</span></span>
                    </div>
                  </div>
                </div>
              </div>
