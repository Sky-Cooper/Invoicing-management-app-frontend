...
export const QuoteEditModal = ({ isOpen, onClose, quote }: Props) => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
        await dispatch(updateQuote({ id: quote.id, data: payload })).unwrap();
        onClose();
    } catch (err) {
          ...
          <div className="flex items-center gap-6 relative z-10">
            ...
            <div>
              ...
              <p className="text-slate-300 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2">
                <ShieldBan size={14} className="text-emerald-400" /> Modification de Devis
              </p>
            </div>
          </div>
               ...
               <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden">
                   <div className="flex justify-between items-end relative z-10">
                       <div className="text-right w-full space-y-1">
                           <p className="text-xs text-slate-400 font-medium">Total HT: <span className="text-white font-bold">{totals.subtotal.toFixed(2)} DH</span></p>
                           <p className="text-xs text-slate-400 font-medium">TVA: <span className="text-white font-bold">{totals.tax.toFixed(2)} DH</span></p>
                           <p className="text-3xl font-black tracking-tighter mt-2">{totals.total.toFixed(2)} <span className="text-lg text-slate-500">DH</span></p>
                       </div>
                   </div>
               </div>
            </div>
